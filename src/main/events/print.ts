...
export class PrintEvent {
  static register() {
    ...
    ipcMain.handle(PrintIPC.TAKE_PARTIAL_SCREENSHOT, async (event, { selection }) => {
      CloseAllWindowsUtil.execute()
      ...
      const contentBounds = senderWindow
        ? senderWindow.getContentBounds()
        : null
      const windowOffset = contentBounds
        ? {
